...
const SkillBadge = ({ name, icon: Icon }) => (
  <motion.div whileHover={{ scale: 1.05, rotate: -2 }} className="sketch-border flex items-center gap-2 px-4 py-2 bg-white text-slate-800 cursor-pointer">
  ...
  return (
   <div className="min-h-screen text-slate-800 paper-pattern overflow-x-hidden selection:bg-yellow-200 relative">
    ...
    <section id="home" className="relative min-h-screen flex items-center justify-center px-6 pt-20 overflow-hidden">
      ...
      <motion.div initial={{ x: -100, rotate: -20 }} animate={{ x: 0, rotate: -5 }} className="absolute top-28 left-6 md:left-20 z-20 hidden md:block group">
        ...
        <div className="max-w-6xl mx-auto text-center z-10 flex flex-col items-center relative mt-12">
          <motion.h1 
            ...
            className="text-7xl md:text-9xl font-black mb-8 tracking-tight text-slate-900 drop-shadow-sm leading-none"
          >
            BUILDING SCALABLE <br/> 
            <span className="marker-highlight px-6 italic inline-block transform -rotate-1">SYSTEMS & AI.</span>
          </motion.h1>
          
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="text-xl md:text-3xl text-slate-700 mb-10 max-w-3xl mx-auto font-bold leading-relaxed">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="flex flex-col sm:flex-row justify-center gap-6">
            <button onClick={() => scrollTo('work')} className="sketch-border px-10 py-5 bg-slate-800 text-white font-bold hover:bg-slate-900 text-2xl flex items-center gap-3"><Rocket size={24} /> Check My Work</button>
            <button onClick={() => scrollTo('contact')} className="sketch-border px-10 py-5 bg-white text-slate-800 font-bold hover:bg-yellow-50 text-2xl flex items-center gap-3"><Mail size={24} /> Let's Chat</button>
          </motion.div>
        </div>
      </section>
        ...
        <div className="max-w-6xl mx-auto w-full grid md:grid-cols-2 gap-16 items-center">
          ...
          <div>
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="mb-12">
              <h2 className="text-5xl font-black mb-4 flex items-center gap-3"><Cpu size={40} className="text-slate-800" /> My Toolkit</h2>
            ...
            <div className="space-y-10">
              ...
              <div className="bg-white p-6 sketch-border relative hover:-rotate-1 transition-transform">
                ...
                <div className="flex flex-wrap gap-4 mt-2">{[["TensorFlow", BrainCircuit], ["LangChain", Sparkles], ["HuggingFace", Bot], ["RAG", Zap]].map(([n, I]) => <SkillBadge key={n} name={n} icon={I} />)}</div>
              </div>
               <div className="bg-white p-6 sketch-border relative hover:rotate-1 transition-transform">
      ...
      <section id="work" className="py-24 px-6 border-t-2 border-b-2 border-slate-800 bg-[#fffdf5] relative">
        ...
        <div className="max-w-6xl mx-auto w-full">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mb-16 text-center">
            <h2 className="text-5xl font-black mb-4 inline-block marker-highlight transform -rotate-1">Things I've Built</h2>
      ...
      <section id="about" className="py-24 px-6">
        <div className="max-w-4xl mx-auto">
           <motion.div initial={{ opacity: 0, rotate: 2 }} whileInView={{ opacity: 1, rotate: 0 }} viewport={{ once: true }} className="sketch-border bg-white p-8 md:p-12 relative">
            ...
            <h2 className="text-4xl font-black mb-8 flex items-center gap-3"><GraduationCap size={40} /> My Story</h2>
            <div className="space-y-8 text-xl">
              <div className="flex gap-6">
                <div className="flex flex-col items-center"><div className="w-4 h-4 bg-slate-800 rounded-full mt-2"></div><div className="w-1 h-full bg-slate-300 my-2"></div></div>
                <div>
                  <h3 className="text-2xl font-bold">NIT Durgapur</h3>
                  <p className="text-slate-600 font-bold">B.Tech CSE (2024 - 2028)</p>
                  <p className="mt-2 text-slate-700">Currently in 3rd Sem. <br/>CGPA: <span className="bg-yellow-200 px-1 border border-black font-bold">8.93</span></p>
                </div>
              </div>
              <div className="flex gap-6">
                <div className="flex flex-col items-center"><div className="w-4 h-4 bg-slate-800 rounded-full mt-2"></div></div>
                <div>
                  <h3 className="text-2xl font-bold">Code Grind</h3>
                  <p className="text-slate-600 font-bold">LeetCode & Problem Solving</p>
                  <p className="mt-2 text-slate-700">Solved 150+ problems. Deep diving into Graphs & DP.</p>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <section id="contact" className="py-24 px-6 mb-20">
        <div className="max-w-3xl mx-auto text-center">
          <h2 className="text-6xl font-black mb-8">Let's Connect!</h2>
          <div className="flex justify-center gap-6 mb-12">
            {[
              ...
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="p-4 sketch-border bg-white hover:bg-slate-800 hover:text-white transition-all transform hover:-rotate-6 hover:scale-110"
              >
                <Icon size={28} />
              </a>
            ))}
